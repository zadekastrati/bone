...
@section('content')
    <x-page-header title="New product" subtitle="Add at least one variant (color + size + stock). SKUs are optional but must be unique when set.">
            ...
            <fieldset class="space-y-4 rounded-3xl border border-ink-200/60 bg-white/95 p-6 shadow-soft">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price') }}" step="0.01" min="0" class="form-input" required>
                    </div>
                    <div class="flex items-end pb-2">
                        <label class="flex items-center gap-2 text-sm font-medium text-ink-800">
                            <input type="hidden" name="is_active" value="0">
                            <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="rounded border-ink-300">
