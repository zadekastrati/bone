...
<body class="min-h-full bg-slate-100/90 font-sans text-base leading-relaxed text-slate-900 antialiased">
    <div
    ...
    >
        ...
        <aside
            class="fixed inset-y-0 left-0 z-50 flex w-64 -translate-x-full flex-col border-r border-slate-800/80 bg-slate-950 text-slate-200 shadow-xl transition-transform duration-300 ease-out lg:static lg:translate-x-0"
            :class="{ '!translate-x-0': sidebarOpen }"
        >
            <div class="flex h-14 items-center gap-2 border-b border-slate-800/90 px-4 lg:h-16">
                <a href="{{ route('home') }}" @click="sidebarOpen = false" class="font-display text-lg font-bold uppercase tracking-mega text-white">
                    {{ config('app.name') }}
                </a>
                <span class="rounded-md bg-rose-500/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-rose-300">Admin</span>
            </div>

            <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Admin">
                <p class="px-3 pb-2 text-[10px] font-bold uppercase tracking-mega text-slate-500">Overview</p>
                ...
                <p class="mt-6 px-3 pb-2 text-[10px] font-bold uppercase tracking-mega text-slate-500">Catalog</p>
                ...
                <p class="mt-6 px-3 pb-2 text-[10px] font-bold uppercase tracking-mega text-slate-500">Sales</p>
                ...
                <p class="mt-6 px-3 pb-2 text-[10px] font-bold uppercase tracking-mega text-slate-500">People</p>
            ...
            <div class="border-t border-slate-800/90 p-3">
                <a href="{{ route('shop.index') }}" @click="sidebarOpen = false" class="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700/80 bg-slate-900/50 px-3 py-2.5 text-xs font-semibold text-slate-300 transition hover:border-slate-600 hover:bg-slate-800 hover:text-white">
                    <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
                    View storefront
                </a>
            </div>
        </aside>
        ...
        <div class="flex min-w-0 flex-1 flex-col">
            <header class="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-slate-200/80 bg-white/90 px-4 shadow-sm backdrop-blur-md sm:px-6 lg:h-16 lg:px-8">
                <div class="flex min-w-0 items-center gap-3">
                    <button
                        type="button"
                        class="inline-flex size-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm lg:hidden"
                    ...
                    <div class="min-w-0">
                        <p class="truncate text-xs font-medium uppercase tracking-mega text-slate-500">Admin</p>
                        <p class="truncate font-display text-lg font-semibold text-slate-900 sm:text-xl">@yield('title', 'Dashboard')</p>
                    </div>
                </div>
                <div class="flex shrink-0 items-center gap-2 sm:gap-4">
                    <span class="hidden max-w-[10rem] truncate text-sm text-slate-600 sm:block" title="{{ auth()->user()->email }}">{{ auth()->user()->name }}</span>
                    <span class="hidden rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-700 sm:inline">{{ auth()->user()->role }}</span>
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50">Log out</button>
                    </form>
                </div>
            </header>
