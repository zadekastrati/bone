...
<body style="margin:0;padding:0;background-color:#f8f5f1;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;opacity:0;">
        {{ __(':name placed order :number for :total.', ['name' => trim($order->shipping_first_name.' '.$order->shipping_last_name), 'number' => $order->order_number, 'total' => config('store.currency_symbol').number_format((float) $order->total, 2)]) }}
                    ...
                    <tr>
                        <td class="bn-card" style="background-color:#ffffff;border:1px solid #eee6de;border-radius:16px;padding:40px;">
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td align="center" style="padding:0 0 24px;">
                                        <p style="margin:0 0 10px;font-size:12px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#8f674b;">{{ __('New order') }}</p>
                                        <p style="margin:0 0 6px;font-size:22px;line-height:1.3;font-weight:700;color:#3a2f28;">{{ config('store.currency_symbol') }}{{ number_format((float) $order->total, 2) }}</p>
                                        <p style="margin:0;font-size:15px;color:#6c584a;">{{ __(':name just placed order :number.', ['name' => trim($order->shipping_first_name.' '.$order->shipping_last_name), 'number' => $order->order_number]) }}</p>
                                    </td>
                                </tr>
                            </table>
                                        ...
                                        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                            <tr>
                                                ...
                                                <td width="50%" style="padding:0 0 0 12px;vertical-align:top;">
                                                    ...
                                                    <p style="margin:0;font-size:14px;font-weight:600;color:#3a2f28;">{{ $order->payment_method->label() }}</p>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:28px;border-top:1px solid #eee6de;">
                                @foreach ($order->items as $item)
                                    <tr>
                                        <td style="padding:16px 0;border-bottom:1px solid #eee6de;vertical-align:top;">
                                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    <td style="vertical-align:top;">
                                                        <p style="margin:0 0 4px;font-size:14px;font-weight:600;color:#3a2f28;">{{ $item->product_name }}</p>
                                                        <p style="margin:0;font-size:13px;color:#866f5d;">{{ $item->color }} · {{ $item->size }} · {{ __('Qty :qty', ['qty' => $item->quantity]) }}</p>
                                                    </td>
                                                    <td width="90" align="right" style="vertical-align:top;white-space:nowrap;">
                                                        <p style="margin:0;font-size:14px;font-weight:700;color:#3a2f28;">{{ config('store.currency_symbol') }}{{ number_format((float) $item->line_total, 2) }}</p>
                                                    </td>
                                                </tr>
                                            </table>
                                        </td>
                                        ...
                                        <table role="presentation" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td style="border-radius:8px;background-color:#5f4332;">
                                                    <a href="{{ route('admin.orders.show', $order) }}" style="display:inline-block;padding:14px 36px;font-size:14px;font-weight:700;letter-spacing:0.04em;color:#ffffff;text-decoration:none;border-radius:8px;">{{ __('View in admin') }}</a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
