@php
    $color = $color ?? null;
    $image = $image ?? $product->thumbnailImage();
    $productUrl = route('shop.product', $color
        ? [$product->category, $product, 'color' => $color]
        : [$product->category, $product]);
@endphp
<article class="store-card-interactive group/card w-36 shrink-0 snap-start overflow-hidden sm:w-44">
    <a href="{{ $productUrl }}" class="relative block aspect-[4/5] overflow-hidden bg-gradient-to-b from-white to-ink-100/80">
        ...
        @if ($image)
            ...
            @else
                <img src="{{ $image->gridUrl() }}" alt="{{ $color ? $product->name.' - '.$color : $product->name }}" loading="lazy" decoding="async" class="size-full object-cover transition duration-700 ease-out group-hover/card:scale-[1.03] motion-reduce:group-hover/card:scale-100">
            @endif
        @else
    ...
    <div class="space-y-1 p-3">
        <p class="ui-eyebrow text-[8px]">{{ $product->category->name }}{{ $color ? ' · '.$color : '' }}</p>
        <h3 class="font-display line-clamp-1 text-[11px] font-bold uppercase leading-snug tracking-wide text-ink-950">
            <a href="{{ $productUrl }}" class="transition hover:text-accent-600">{{ $product->name }}</a>
        </h3>
