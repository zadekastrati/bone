...
<div class="space-y-16 lg:space-y-20">
    @if ($isSearch)
        <section class="scroll-mt-28" aria-labelledby="search-results-heading">
            <h2 id="search-results-heading" class="sr-only">{{ __('Matching products') }}</h2>
            @if ($searchResults->isEmpty())
                <div class="rounded-3xl border border-zinc-200/70 bg-gradient-to-b from-zinc-50/95 to-white px-6 py-14 text-center shadow-soft ring-1 ring-zinc-900/[0.04]">
                    <p class="font-display text-lg font-bold uppercase tracking-wide text-ink-950">{{ __('No matches') }}</p>
                    <p class="mt-2 text-sm text-ink-600 text-pretty">{{ __('Try a shorter term or browse categories below.') }}</p>
