@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="pagination-nav pagination-nav--simple">
        <div class="pagination-nav__surface pagination-nav__surface--simple flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="pagination-nav__simple-btn sm:ml-auto">
                    {{ __('pagination.next') }}
            ...
            @else
                <span class="pagination-nav__simple-btn pagination-nav__simple-btn--disabled sm:ml-auto">
                    {{ __('pagination.next') }}
                    <svg class="size-5 shrink-0" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" /></svg>
                </span>
            @endif
