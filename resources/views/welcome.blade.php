...
@section('content')
    ...
    <section class="relative flex min-h-[85vh] flex-col justify-end overflow-hidden bg-hero-radial pb-20 pt-28 text-white sm:min-h-[90vh] sm:justify-center sm:pb-28 sm:pt-20 lg:pt-24">
        <div class="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(9,9,11,0.2)_0%,rgba(9,9,11,0.85)_100%)]"></div>
        ...
        <div class="relative mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
            <p class="text-[11px] font-bold uppercase tracking-[0.25em] text-accent-300/95 sm:text-xs">Women&apos;s performance · gym to street</p>
            <h1 class="font-display mt-5 max-w-4xl text-5xl font-bold uppercase leading-[0.9] tracking-tight text-balance sm:text-7xl md:text-8xl lg:text-9xl">
                Become your<br class="hidden sm:inline" /> personal best
            </h1>
            <p class="mx-auto mt-8 max-w-xl text-base leading-relaxed text-white/65 text-pretty sm:mx-0 sm:text-lg sm:leading-relaxed">
            ...
            <div class="mt-10 flex flex-wrap items-center gap-3 sm:mt-12 sm:gap-4">
                <a href="{{ route('shop.index') }}" class="inline-flex min-h-[3rem] items-center justify-center bg-white px-10 py-3.5 text-xs font-bold uppercase tracking-mega text-ink-950 shadow-float transition hover:bg-white/95 active:scale-[0.98] sm:px-12">
                    Shop women&apos;s
                </a>
                <a href="{{ route('register') }}" class="btn-outline-light min-h-[3rem] px-10 py-3.5 text-xs sm:px-12">Join the list</a>
            </div>
        </div>
    </section>

    {{-- Category strip: editorial labels + quick links --}}
    <section class="border-y border-white/10 bg-zinc-950 px-4 py-4 sm:px-6">
        <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 sm:flex-row sm:gap-8">
            <p class="text-center text-[10px] font-bold uppercase tracking-mega text-white/50 sm:text-left">Trending</p>
            <div class="flex flex-wrap items-center justify-center gap-2 sm:justify-end">
                @foreach (['New arrivals' => route('shop.index') . '#new', 'Shop all' => route('shop.index'), 'Sports bras' => route('shop.index'), 'Layers' => route('shop.index')] as $label => $href)
                    <a href="{{ $href }}" class="rounded-full border border-white/15 bg-white/[0.04] px-4 py-2 text-[10px] font-bold uppercase tracking-mega text-white/90 transition hover:border-accent-400/50 hover:bg-white/[0.08]">{{ $label }}</a>
                @endforeach
            </div>
        </div>
    </section>
    ...
    {{-- Shop by category --}}
    <section class="bg-gradient-to-b from-zinc-100 via-zinc-50 to-white px-4 py-16 sm:px-6 lg:py-24">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="mt-12 grid gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4 lg:gap-6">
                @foreach ([
                    ['label' => 'Leggings', 'sub' => 'Sculpt & sweat', 'href' => route('shop.index'), 'tone' => 'from-zinc-800 to-zinc-950'],
                    ['label' => 'Sports bras', 'sub' => 'High support', 'href' => route('shop.index'), 'tone' => 'from-accent-900/90 to-ink-950'],
                    ['label' => 'Tops', 'sub' => 'Crops & tanks', 'href' => route('shop.index'), 'tone' => 'from-neutral-700 to-neutral-900'],
                    ['label' => 'Layers', 'sub' => 'Hoodies & jackets', 'href' => route('shop.index'), 'tone' => 'from-zinc-700 to-zinc-900'],
                ] as $tile)
                    <a
                        href="{{ $tile['href'] }}"
                        class="group relative flex min-h-[260px] flex-col justify-end overflow-hidden rounded-3xl bg-gradient-to-br p-7 text-white shadow-float ring-1 ring-white/10 transition duration-500 ease-out-expo motion-reduce:transition-none {{ $tile['tone'] }} motion-safe:hover:-translate-y-1 motion-safe:hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.45)] motion-safe:hover:ring-accent-400/30"
                    >
    ...
    {{-- Editorial band --}}
    <section class="relative overflow-hidden bg-ink-950 px-4 py-24 text-white sm:px-6 lg:py-32">
        <div class="pointer-events-none absolute -right-20 -top-20 size-[28rem] rounded-full bg-accent-500/12 blur-3xl sm:size-[36rem]"></div>
        <div class="pointer-events-none absolute bottom-0 left-0 size-64 rounded-full bg-white/5 blur-3xl"></div>
        <div class="relative mx-auto max-w-3xl text-center">
            <p class="text-[10px] font-bold uppercase tracking-mega text-accent-300/90">Why bone</p>
            <h2 class="font-display mt-4 text-3xl font-bold uppercase tracking-wide text-balance sm:text-5xl lg:text-6xl">Built in the weight room mindset</h2>
            <p class="mx-auto mt-8 text-lg leading-relaxed text-white/55 text-pretty sm:text-xl">
