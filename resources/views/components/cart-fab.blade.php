...
<a
    ...
    x-data="{ init() { $store.cart.count = {{ (int) $count }}; } }"
    ...
    :aria-label="$store.cart.count > 0 ? 'Shopping bag, ' + $store.cart.count + ' items' : 'Shopping bag, empty'"
>
    ...
    <span
        ...
        class="absolute -right-1 -top-1 flex min-h-[1.35rem] min-w-[1.35rem] items-center justify-center rounded-full bg-ink-900 px-1 text-[11px] font-bold leading-none text-white ring-2 ring-white"
    ></span>
</a>
