    ...
    public function store(StoreCheckoutRequest $request): RedirectResponse
    {
        ...
        if ($validated['payment_method'] === PaymentMethod::Card) {
            try {
                ...
                // hppUrl from the gateway is just the base card-entry page —
                // the id/password have to be appended as query params for it
                // to know which order this is (see integration doc §4.1.3).
                $redirectUrl = $gatewayOrder['hppUrl'].'?'.http_build_query([
                    'id' => $gatewayOrder['id'],
